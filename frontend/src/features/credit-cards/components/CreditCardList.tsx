...
export function CreditCardList({ creditCards, onEdit, onDeactivate }: CreditCardListProps) {
  if (creditCards.length === 0) {
    return (
      <div className="text-center py-12 bg-[var(--bg-secondary)] rounded-xl border border-dashed border-[var(--border-color)]">
        ...
        <p className="text-[var(--text-muted)]">新增您的第一張信用卡以開始追蹤分期與未繳金額</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {creditCards.map((card) => (
        <div key={card.id} className="card-dark p-5 hover:border-[var(--border-hover)] transition-all group relative">
