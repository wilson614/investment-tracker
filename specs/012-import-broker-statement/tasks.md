...
- [ ] T149 Add resumable import query contracts (`sessionId`, checkpoint cursor, row-level status) in `backend/src/InvestmentTracker.Application/DTOs/RequestDtos.cs` and `backend/src/InvestmentTracker.Application/DTOs/StockImportDtos.cs`
- [ ] T150 Implement resumable import-session query use case with checkpoint reconstruction in `backend/src/InvestmentTracker.Application/UseCases/StockTransactions/QueryStockImportSessionUseCase.cs` and `backend/src/InvestmentTracker.Application/UseCases/StockTransactions/ExecuteStockImportUseCase.cs`
