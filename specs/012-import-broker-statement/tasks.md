...
- [x] T146 Unify import date canonicalization rules (trade date, baseline date, seeded adjustment date) in `backend/src/InvestmentTracker.Application/UseCases/StockTransactions/StockImportParser.cs` and `backend/src/InvestmentTracker.Application/UseCases/StockTransactions/ExecuteStockImportUseCase.cs`
- [x] T147 Persist normalized canonical dates into preview/execute session snapshots to guarantee cross-step date consistency in `backend/src/InvestmentTracker.Application/UseCases/StockTransactions/PreviewStockImportUseCase.cs` and `backend/src/InvestmentTracker.Application/DTOs/StockImportDtos.cs`
- [x] T148 Add regression coverage for date-rule consistency (year-boundary `1/1`, FX lookup date, same-day ordering) in `backend/tests/InvestmentTracker.Application.Tests/UseCases/StockTransactions/ExecuteStockImportBalanceActionTests.cs` and `backend/tests/InvestmentTracker.API.Tests/Controllers/StockTransactionsImportControllerTests.cs`
- [x] T149 Add resumable import query contracts (`sessionId`, checkpoint cursor, row-level status) in `backend/src/InvestmentTracker.Application/DTOs/RequestDtos.cs` and `backend/src/InvestmentTracker.Application/DTOs/StockImportDtos.cs`
- [x] T150 Implement resumable import-session query use case with checkpoint reconstruction in `backend/src/InvestmentTracker.Application/UseCases/StockTransactions/QueryStockImportSessionUseCase.cs` and `backend/src/InvestmentTracker.Application/UseCases/StockTransactions/ExecuteStockImportUseCase.cs`
- [x] T151 Add resumable import query API endpoint and contract tests in `backend/src/InvestmentTracker.API/Controllers/StockTransactionsController.cs` and `backend/tests/InvestmentTracker.API.Tests/Controllers/StockTransactionsImportControllerTests.cs`
- [x] T152 Add preview/execute performance benchmark coverage (500/2000 rows; warm/cold path) in `backend/tests/InvestmentTracker.Application.Tests/UseCases/StockTransactions/PreviewStockImportPerformanceTests.cs` and `backend/tests/InvestmentTracker.Application.Tests/UseCases/StockTransactions/ExecuteStockImportPerformanceTests.cs`
- [x] T153 Optimize import execution data access (prefetch holdings/cash context and reduce per-row round trips) in `backend/src/InvestmentTracker.Application/UseCases/StockTransactions/ExecuteStockImportUseCase.cs` and `backend/src/InvestmentTracker.Application/UseCases/StockTransactions/CreateStockTransactionUseCase.cs`
- [x] T154 Add post-optimization performance threshold assertions and diagnostics evidence hooks in `backend/tests/InvestmentTracker.Application.Tests/UseCases/StockTransactions/PreviewStockImportPerformanceTests.cs` and `specs/012-import-broker-statement/quickstart.md`
- [x] T155 Introduce current-holdings projection model fields (shares/cost/market-value source) in `backend/src/InvestmentTracker.Application/DTOs/PerformanceDtos.cs` and `backend/src/InvestmentTracker.Domain/Services/PortfolioCalculator.cs`
- [x] T156 Align yearly/aggregate performance composition with current-holdings model for imported baseline positions in `backend/src/InvestmentTracker.Application/Services/HistoricalPerformanceService.cs` and `backend/src/InvestmentTracker.Application/UseCases/Performance/CalculateAggregateYearPerformanceUseCase.cs`
- [x] T157 Implement automatic sell-before-buy resolution (same-date symbol netting before balance-action fallback) in `backend/src/InvestmentTracker.Application/UseCases/StockTransactions/ExecuteStockImportUseCase.cs` and `backend/src/InvestmentTracker.Application/DTOs/RequestDtos.cs`
- [x] T158 Add automatic sell-before-buy regression coverage for default behavior and per-row override precedence in `backend/tests/InvestmentTracker.Application.Tests/UseCases/StockTransactions/ExecuteStockImportBalanceActionTests.cs` and `backend/tests/InvestmentTracker.API.Tests/Controllers/StockTransactionsImportControllerTests.cs`
- [x] T159 Re-scope annual performance metrics: remove annual XIRR computation from yearly performance flows and ensure the Performance page uses only Modified Dietz / TWR / Total Return for yearly analysis in `backend/src/InvestmentTracker.Application/Services/HistoricalPerformanceService.cs`, `backend/src/InvestmentTracker.Application/DTOs/PerformanceDtos.cs`, `backend/src/InvestmentTracker.Application/UseCases/Performance/CalculateAggregateYearPerformanceUseCase.cs`, `frontend/src/types/index.ts`, and `frontend/src/pages/Performance.tsx`
- [x] T160 Refactor frontend import/performance UX for resumable query state, auto sell-before-buy guidance, and XIRR status messaging in `frontend/src/components/import/StockImportButton.tsx`, `frontend/src/components/import/CSVImportModal.tsx`, and `frontend/src/pages/Performance.tsx`
- [x] T161 Backfill this round verification evidence (date consistency, resumable query, performance optimization, holdings/XIRR results) in `specs/012-import-broker-statement/quickstart.md` and `specs/012-import-broker-statement/tasks.md`
- [x] T162 Fix Day-1 snapshot valuation fallback for missing/zero historical prices by using seeded cost basis in `backend/src/InvestmentTracker.Infrastructure/Services/TransactionPortfolioSnapshotService.cs` and `backend/src/InvestmentTracker.Domain/Services/PortfolioCalculator.cs`
- [x] T163 [P] Add infrastructure regressions for non-trading anchor and missing/zero price scenarios to keep Day-1 snapshot value on seeded cost basis in `backend/tests/InvestmentTracker.Infrastructure.Tests/Services/TransactionPortfolioSnapshotServiceTests.cs`
- [x] T164 [P] Add application regressions to prevent TWR collapsing to `-100%` when InitialBalance is external CF and offset Spend remains internal in `backend/tests/InvestmentTracker.Application.Tests/HistoricalPerformanceServiceReturnTests.cs`
- [x] T165 Run focused QA verification for Day-1 fallback and import-performance regressions in `backend/tests/InvestmentTracker.Infrastructure.Tests/Services/TransactionPortfolioSnapshotServiceTests.cs`, `backend/tests/InvestmentTracker.Application.Tests/HistoricalPerformanceServiceReturnTests.cs`, and `backend/tests/InvestmentTracker.API.Tests/Controllers/StockTransactionsImportControllerTests.cs`
- [x] T166 Complete code-review gate for Day-1 zero-price fallback closure across implementation and regression files in `backend/src/InvestmentTracker.Infrastructure/Services/TransactionPortfolioSnapshotService.cs`, `backend/src/InvestmentTracker.Domain/Services/PortfolioCalculator.cs`, `backend/tests/InvestmentTracker.Infrastructure.Tests/Services/TransactionPortfolioSnapshotServiceTests.cs`, and `backend/tests/InvestmentTracker.Application.Tests/HistoricalPerformanceServiceReturnTests.cs`
- [x] T167 Fix strict Day-1 zero-value fallback guard by adding transaction-level seeded-adjustment cost recovery in `backend/src/InvestmentTracker.Infrastructure/Services/TransactionPortfolioSnapshotService.cs`
- [x] T168 [P] Add infrastructure regression for `AverageCostPerShareHome == 0` seeded-adjustment Day-1 scenario in `backend/tests/InvestmentTracker.Infrastructure.Tests/Services/TransactionPortfolioSnapshotServiceTests.cs`
